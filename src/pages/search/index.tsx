...
export function SearchResult() {
  ...
  return (
    <div className="w-full h-full space-y-4 bg-background">
      <Section
        title={`Kết quả (${searchResult.length})`}
        className="h-full flex flex-col overflow-y-auto pb-16 fade-in-up"
      >
        {searchResult.length ? (
...
export function RecommendedProducts() {
  ...
  return (
    <Section title="Gợi ý sản phẩm" className="fade-in-up">
      <div className="py-4 px-4 pb-6 flex space-x-4 overflow-x-auto">
        {recommendedProducts.map((product) => (
          <div
            key={product.id}
            className="flex-none"
            style={{ flexBasis: "calc((100vw - 64px) / 2)" }}
          >
